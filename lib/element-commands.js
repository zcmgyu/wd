...
elementCommands.sendKeys = function(keys, cb) {
  ...
  var path = keys.join("");
  // TODO: Remove for Dockyard
  // _isLocalFile(path, function(err, isLocalFile) {
  //   if (err) {
  //     return cb(err);
  //   }
  //   if (isLocalFile) {
  //     commands.uploadFile.apply(_this.browser, [
  //       path,
  //       function(err, distantFilePath) {
  //         if (err) {
  //           return cb(err);
  //         }
  //         return commands.type.apply(_this.browser, [
  //           _this,
  //           distantFilePath,
  //           cb
  //         ]);
  ...
  //   }
  // });
  // TODO: Add for dockyard sendKeys
  commands.type.apply(_this.browser, [_this, keys, cb]);
};
...
elementCommands.replaceKeys = function(keys, cb) {
  deprecator.warn(
    "element.replaceKeys",
    "element.replaceKeys has been deprecated, use element.setText instead."
  );
...
elementCommands.doubleclick = function(cb) {
  return commands.moveTo.apply(this.browser, [
    this,
    function(err) {
      if (err) {
        return cb(err);
...
_.each(utils.elementFuncTypes, function(type) {
  ...
  elementCommands["element" + utils.elFuncSuffix(type)] = function(value, cb) {
    elementCommands.element.apply(this, [
      utils.elFuncFullType(type),
      value,
      cb
    ]);
  };
  ...
  elementCommands["elements" + utils.elFuncSuffix(type)] = function(value, cb) {
    elementCommands.elements.apply(this, [
      utils.elFuncFullType(type),
      value,
      cb
    ]);
...
/**
 * element.getElementScreenshot(cb) -> cb(err)
 *
 * @jsonWire POST /session/:sessionId/element/:id/getElementScreenshot
 */
elementCommands.getScreenshot = function(cb) {
  commands.getElementScreenshot.apply(this.browser, [this, cb]);
};
